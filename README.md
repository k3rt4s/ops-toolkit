...
| `scripts\it-operations\windows-hardening\`    | Workstation idle-lock, sleep, browser credential, BitLocker, local admin posture, plus system-level TLS/privacy/bloatware hardening and its compliance check |
...
$scopeExclusions = @(@{ Target = 'pc02'; Reason = 'Retired asset awaiting inventory removal' })
& .\scripts\reporting\Export-SecurityControlEvidencePack.ps1 -ComputerName pc01,pc02 -ScopeExclusion $scopeExclusions
```

The pack is a point-in-time technical evidence snapshot, not a NIST maturity rating,
ISO conformity decision, audit, or certification report. Each control records its
intended, attempted, observed, failed, and explicitly excluded population; limitations;
exact evidence paths; observation time; and SHA-256 hashes. `run-context.json` records
the toolkit revision and redacted collection context, while `evidence-manifest.csv`
and `.json` inventory the generated artifacts. An unread required source remains
`NotAssessed`; it is never scored or presented as a pass.

The evidence pack requires PowerShell 7. When management-plane reconciliation is used,
it snapshots sanitized copies of the manifest and each authority CSV under `inputs\`,
hashes them, removes explicit scope exclusions before grading, and requires at least
two readable required authorities including the supplied Defender inventory before it
can report clean estate endpoint coverage.
...
Grade whether the tenant could support an incident investigation today, including
two subscriptions' Activity Log export and break-glass FIDO2 coverage:

```powershell
pwsh -File .\scripts\entra\Export-CloudIncidentReadiness.ps1 -Connect -ConnectAzure -SubscriptionId <sub-guid-1>,<sub-guid-2> -BreakGlassUpn breakglass1@contoso.com,breakglass2@contoso.com
```
...
```powershell
pwsh -File .\scripts\it-operations\utilities\Find-LegacyApiUsage.ps1 -Path C:\Scripts -Severity Broken
```
...
```powershell
pwsh -File .\scripts\it-operations\windows-hardening\Test-WindowsHardeningState.ps1 -ProbeEndpoint 'www.example.com:443'
pwsh -File .\scripts\it-operations\windows-hardening\Test-WindowsHardeningState.ps1 -ComputerName srv01,srv02 -FailOnDrift
```
...
> **Folder note:** `scripts\it-operations\windows-hardening\` mixes two run cadences. System-level
> hardening (TLS cipher policy, Windows 11 privacy/telemetry, AppX bloatware removal) is typically
> run once at build or provisioning time. Operator posture scripts (idle-lock, sleep, browser
> credential hardening) are run and rolled back as workload needs change.
...
| `scripts\it-operations\utilities\Join-ApplicationsWithEndpointSites.ps1`             | Rebuilt CSV join utility with configurable join columns, case handling, matched/unmatched reports, duplicate-key summaries, and output paths under reports.                                                                                                                                                                                                                                                                           |
...
| `scripts\it-operations\windows-hardening\Set-WindowsSchannelTlsHardening.ps1`        | Renamed and rebuilt Schannel TLS hardening with `-WhatIf`, plan reports, registry backups, and summaries.                                                                                                                                                                                                                                                                                                                             |
| `scripts\it-operations\windows-hardening\Set-Windows11PrivacyHardening.ps1`          | Renamed and rebuilt Windows 11 privacy/AI hardening with `-WhatIf`, rollback, plan/state reports, registry backups, and summaries.                                                                                                                                                                                                                                                                                                    |
| `scripts\it-operations\windows-hardening\Remove-WindowsProvisionedBloatwareApps.ps1` | Rebuilt Windows 11 AppX bloatware removal with clean data lists, `-WhatIf`, rollback guidance, inventory/plan/state reports, and protected package enforcement.                                                                                                                                                                                                                                                                       |
...
| `scripts\it-operations\windows-file-cleanup\Invoke-DiskSpaceReclaim.ps1`             | New script reclaiming pip cache, Docker build cache and dangling images, Recycle Bin, and WinSxS component store by default, with npm, torch, pre-commit, Codex runtime, NVIDIA shader, Playwright, Hugging Face, stopped-container, unused-volume, superseded image-tag, Windows Update cache, and Docker virtual-disk compaction targets available by name, plus `-WhatIf`, target selection, admin gating, and plan/state reports. |
...
| `scripts\entra\Export-CloudIncidentReadiness.ps1`                                    | New read-only collector grading whether this tenant's logging, roles, and identity controls could support an incident investigation today across nine checks (audit log ingestion, Entra and subscription log export, workspace retention, Graph activity logs, responder role eligibility, break-glass FIDO2 coverage, user consent, Conditional Access coverage of the device code flow). Feeds control IR-02 in the evidence pack. |
...
| `scripts\reporting\Export-SecurityControlEvidencePack.ps1`                           | Assembles technical control evidence with per-control scope, population counts, exclusions with reasons, provenance, freshness, limitations, and artifact hashes. Can combine Defender management-plane inventory with coverage reconciliation for estate endpoint-protection evidence while preserving NotAssessed for unread required sources.                                                                                      |
| `scripts\it-operations\utilities\Find-LegacyApiUsage.ps1`                            | New read-only scanner for retired and soon-to-be-retired Microsoft APIs (MSOnline, AzureAD, AzureRM, ADAL, EWS, Exchange Online `-Credential`, Application Impersonation, Get-MessageTrace, Send-MailMessage), reporting deadline, replacement, and whether the hit is in a comment.                                                                                                                                                  |
...
| `scripts\it-operations\utilities\compare_folders.py`                                 | New bidirectional BLAKE3 folder comparison with multiprocessing, four CSV/text output sets, and an optional SHA-256 verification pass.                                                                                                                                                                                                                                                                                                |
