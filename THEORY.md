...
generated syntax, as was true of 30 of 31 scripts for months. Custom sections go inside
`.DESCRIPTION` or `.NOTES` as labelled text.
...
are pwsh-7 only, so the three scripts using them carry `#requires -Version 7`.
...
**Report writes and report directories carry `-WhatIf:$false`.** Creating the directory
and writing the plan is the preview, not the change being previewed. Without it
`New-Item` is suppressed, the following `Resolve-Path` throws, and the script cannot be
rehearsed at all unless the directory already exists.

**Undetermined is never folded into a pass.** A check that could not run is reported as
not having run: the evidence pack counts NotAssessed separately from NotMet, because
...
Scripts are not modules, so tests parse their functions out and import them **globally**,
because Pester runs `BeforeAll` and `It` in different scopes. Fixtures must plant the
null shapes a real service returns. Tests pass vacuously the same way scripts fail
quietly: `foreach` over a null summary asserts nothing and `-Not -Contain` on a
misspelled column always succeeds, so assert the count first.
...
ahead of the real one, then check the machine rather than the test result.

**Every state-changing script is tested as a pair**, `-WhatIf` and executing. Alone,
"`-WhatIf` changed nothing" is unfalsifiable: a broken script passes it too. Detail in
`tests\README.md`.
