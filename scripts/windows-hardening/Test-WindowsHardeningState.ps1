...
foreach ($name in $Target) {
    ...
    if (-not (Test-Path -LiteralPath $spec.Path)) {
        ...
        $scriptResults.Add([pscustomobject]@{ Target = $name; Status = 'ScriptMissing'; ItemCount = 0; CompliantCount = 0; DriftCount = 0; Note = $spec.Path })
        continue
    }
    ...
    try {
        ...
        $scriptResults.Add([pscustomobject]@{ Target = $name; Status = 'PlanFailed'; ItemCount = 0; CompliantCount = 0; DriftCount = 0; Note = $_.Exception.Message })
        continue
    }
    ...
    if (-not $planFile) {
        ...
        $scriptResults.Add([pscustomobject]@{ Target = $name; Status = 'NoPlanFile'; ItemCount = 0; CompliantCount = 0; DriftCount = 0; Note = $planDirectory })
        continue
    }
    ...
    foreach ($item in $planItems) {
        ...
        # NoChange is the plan saying the machine already holds the desired value.
        # Anything else is the hardening not being in effect right now.
        $state = switch ($action) {
            'NoChange' { 'Compliant' }
        ...
        $complianceRecords.Add([pscustomobject]@{
                ...
                Item = if (Get-OpsPropertyValue -InputObject $item -Name 'Target') {
                    ...
                    (@(
                        [string](Get-OpsPropertyValue -InputObject $item -Name 'RegistryPath')
                        [string](Get-OpsPropertyValue -InputObject $item -Name 'ValueName')
                    ) | Where-Object { $_ }) -join '\'
                }
    ...
    $forTarget = @($complianceRecords | Where-Object { $_.Target -eq $name })
